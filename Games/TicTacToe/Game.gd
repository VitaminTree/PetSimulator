...
const cross = preload("res://Games/TicTacToe/X_icon.png")
const nought = preload("res://Games/TicTacToe/O_icon.png")
const blank = preload("res://Games/TicTacToe/Blank_icon_128px.png")

@onready var grid = $TokenContainer
@onready var status = $Label
@onready var winnings = $Label2
...
func endstate(winning_state: int) -> void:
	...
	if winning_state > 0:
		...
		status.text = "TIE"
		value = 50
	winnings.text = "You have earned %d coins" % value
	status.show()
	winnings.show()
	GameManager.player.inventory.add_item("Money", value)
